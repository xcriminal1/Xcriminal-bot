...
read_when:
  ...
  - Deciding: standalone CLI vs deep Xcriminal integration
...
Recommendation: **deep integration in Xcriminal**, but keep a separable core library.

### Why integrate into Xcriminal?
- Xcriminal already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `Xcriminal memory recall "…" --k 25 --since 30d`
  - `Xcriminal memory reflect --since 7d`
