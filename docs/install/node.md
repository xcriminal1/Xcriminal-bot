...
read_when:
  - "You installed Xcriminal but `Xcriminal` is “command not found”"
...
Xcriminal’s runtime baseline is **Node 22+**.

If you can run `npm install -g Xcriminal@latest` but later see `Xcriminal: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
