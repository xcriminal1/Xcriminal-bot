...
- Jobs persist under `~/.Xcriminal/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.Xcriminal/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.Xcriminal/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.Xcriminal/cron/jobs.json",
...
Xcriminal cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
Xcriminal cron edit <jobId> --agent ops
Xcriminal cron edit <jobId> --clear-agent
```
```
...
```bash
Xcriminal system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`Xcriminal system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `Xcriminal_SKIP_CRON`.
