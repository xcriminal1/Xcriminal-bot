...
- **gateway host** → `Xcriminal` process on the gateway machine
...
`~/.Xcriminal/exec-approvals.json` directly.

CLI: `Xcriminal approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
