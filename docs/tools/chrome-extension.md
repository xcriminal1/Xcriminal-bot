---
summary: "Chrome extension: let Xcriminal drive your existing Chrome tab"
read_when:
...
The Xcriminal Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
Xcriminal then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Xcriminal release (npm package) as static files. There is no separate “build” step.

After upgrading Xcriminal:
- Re-run `Xcriminal browser extension install` to refresh the installed files under your Xcriminal state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Xcriminal ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Xcriminal to control.
...
- `ON`: attached; Xcriminal can drive that tab.
...
Debugging: `Xcriminal sandbox explain`
...
`Xcriminal browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `Xcriminal browser extension install` first to copy the extension to a stable location under your Xcriminal state directory.
