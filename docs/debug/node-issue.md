...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Xcriminal
---

# Node + tsx "__name is not a function" crash

## Summary
Running Xcriminal via Node with `tsx` fails at startup with:

```
[Xcriminal] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `Xcriminal status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch Xcriminal.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node Xcriminal.mjs status` works on Node 25.
