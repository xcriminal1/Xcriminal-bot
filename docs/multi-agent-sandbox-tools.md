...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `Xcriminal sandbox explain`.
...
Legacy `agent.*` configs are migrated by `Xcriminal doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=Xcriminal.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${Xcriminal_STATE_DIR:-$HOME/.Xcriminal}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
