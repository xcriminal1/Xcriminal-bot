...
This document explains how Xcriminal manages sessions end-to-end:
...
Xcriminal is designed around a single **Gateway process** that owns session state.
...
Xcriminal persists sessions in two layers:
...
- Transcripts: `~/.Xcriminal/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Xcriminal resolves these via `src/config/sessions.ts`.
...
Xcriminal intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Xcriminal consumes the events, but Pi decides when to compact).
...
Xcriminal also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Xcriminal bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Xcriminal leaves it alone.
...
- `Xcriminal status` (CLI)
- `Xcriminal sessions` / `sessions --json`
...
Xcriminal supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Xcriminal strips/suppresses this in the delivery layer.

As of `2026.1.10`, Xcriminal also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Xcriminal uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Xcriminal’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `Xcriminal status`.
