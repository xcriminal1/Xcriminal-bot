...
Xcriminal features that can generate provider usage or paid API calls.
...
- `Xcriminal status --usage` and `Xcriminal channels list` show provider **usage windows**
...
Xcriminal can pick up credentials from:
...
### 8) Model scan / probe
`Xcriminal models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
