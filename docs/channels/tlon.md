...
Tlon is a decentralized messenger built on Urbit. Xcriminal connects to your Urbit ship and can
...
```bash
Xcriminal plugins install @Xcriminal/tlon
```

Local checkout (when running from a git repo):

```bash
Xcriminal plugins install ./extensions/tlon
```
...
Use these with `Xcriminal message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Xcriminal replies in-thread.
