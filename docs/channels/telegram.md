...
- If Telegram rejects the HTML payload, Xcriminal retries the same message as plain text.

## Commands (native + custom)
Xcriminal registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; Xcriminal does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `Xcriminal logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). Xcriminal can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. Xcriminal:
...
Private chats can include `message_thread_id` in some edge cases. Xcriminal keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `Xcriminal pairing approve telegram <CODE>`
...
2) Run `Xcriminal logs --follow` and look for `from.id`.
...
Xcriminal defaults to audio files for backward compatibility.
...
Xcriminal supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, Xcriminal handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, Xcriminal caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.Xcriminal/telegram/sticker-cache.json`
...
Xcriminal uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, Xcriminal:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by Xcriminal)
...
- Example: `Xcriminal message send --channel telegram --target 123456789 --message "hi"`.
...
- `Xcriminal channels status` shows a warning when config expects unmentioned group messages.
- `Xcriminal channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `Xcriminal logs --follow` (look for "skipping group message")
...
- Upgrade to a Xcriminal build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
