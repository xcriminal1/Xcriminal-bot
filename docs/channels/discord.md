...
## Goals
- Talk to Xcriminal via Discord DMs or guild channels.
...
3. Configure Xcriminal with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `Xcriminal pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Xcriminal in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Xcriminal needs
...
Discord uses numeric ids everywhere; Xcriminal config prefers ids.
...
### Troubleshooting
- First: run `Xcriminal doctor` and `Xcriminal channels status --probe` (actionable warnings + quick audits).
...
- On startup, Xcriminal resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror Xcriminal’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Xcriminal enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`Xcriminal gateway --force`) after confirming no other processes own the Discord session.
