...
```bash
Xcriminal plugins install @Xcriminal/msteams
```

Local checkout (when running from a git repo):
```bash
Xcriminal plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Xcriminal will offer the local install path automatically.
...
3) Configure Xcriminal with those credentials.
...
## Goals
- Talk to Xcriminal via Teams DMs, group chats, or channels.
...
- On startup, Xcriminal resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.Xcriminal/Xcriminal.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Xcriminal, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `Xcriminal-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `Xcriminal plugins install @Xcriminal/msteams`
   - From a local checkout: `Xcriminal plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Xcriminal in Teams", "full": "Xcriminal in Teams" },
...
Xcriminal handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Xcriminal only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/XcriminalShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Xcriminal sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `Xcriminal message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.Xcriminal/msteams-polls.json`.
...
**CLI:**
```bash
Xcriminal message send --channel msteams \
...
Xcriminal message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
Xcriminal message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
Xcriminal message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
Xcriminal message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
