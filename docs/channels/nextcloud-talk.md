...
```bash
Xcriminal plugins install @Xcriminal/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
Xcriminal plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Xcriminal" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `Xcriminal pairing list nextcloud-talk`
  - `Xcriminal pairing approve nextcloud-talk <CODE>`
