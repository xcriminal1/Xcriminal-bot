...
3) Configure Xcriminal with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for Xcriminal + `imsg` (Messages DB access).
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. Xcriminal only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. Xcriminal can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, Xcriminal attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - Xcriminal gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.Xcriminal/scripts/imsg-ssh",
...
Example wrapper (`~/.Xcriminal/scripts/imsg-ssh`):
```bash
#!/usr/bin/env bash
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.Xcriminal/Xcriminal.json` (it often contains tokens).
...
- Approve via:
  - `Xcriminal pairing list imessage`
  - `Xcriminal pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, Xcriminal treats that thread as a “group” for:
