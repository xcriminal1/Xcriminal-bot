...
Matrix is an open, decentralized messaging protocol. Xcriminal connects as a Matrix **user**
...
```bash
Xcriminal plugins install @Xcriminal/matrix
```

Local checkout (when running from a git repo):

```bash
Xcriminal plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `Xcriminal plugins install @Xcriminal/matrix`
   - From a local checkout: `Xcriminal plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Xcriminal calls the same
     login endpoint, stores the access token in `~/.Xcriminal/credentials/matrix/credentials.json`,
...
- On first connection, Xcriminal requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Xcriminal logs a warning.
...
`~/.Xcriminal/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `Xcriminal pairing list matrix`
  - `Xcriminal pairing approve matrix <CODE>`
...
- On startup, Xcriminal resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
