...
read_when:
  - Setting up Twitch chat integration for Xcriminal
---
# Twitch (plugin)

Twitch chat support via IRC connection. Xcriminal connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
Xcriminal plugins install @Xcriminal/twitch
```

Local checkout (when running from a git repo):

```bash
Xcriminal plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `Xcriminal_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "Xcriminal",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use Xcriminal_TWITCH_ACCESS_TOKEN env var)
...
```bash
Xcriminal_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
