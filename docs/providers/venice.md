---
summary: "Use Venice AI privacy-focused models in Xcriminal"
read_when:
  - You want privacy-focused inference in Xcriminal
...
```bash
Xcriminal onboard --auth-choice venice-api-key
```
...
```bash
Xcriminal chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Xcriminal shows all available Venice models. Pick based on your needs:
...
```bash
Xcriminal models set venice/claude-opus-45
Xcriminal models set venice/llama-3.3-70b
```

List all available models:

```bash
Xcriminal models list | grep venice
```

## Configure via `Xcriminal configure`

1. Run `Xcriminal configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Xcriminal automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
Xcriminal chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
Xcriminal chat --model venice/claude-opus-45

# Use uncensored model
Xcriminal chat --model venice/venice-uncensored

# Use vision model with image
Xcriminal chat --model venice/qwen3-vl-235b-a22b

# Use coding model
Xcriminal chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `Xcriminal models list` to see currently available models. Some models may be temporarily offline.
