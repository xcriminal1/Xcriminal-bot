---
summary: "Use Synthetic's Anthropic-compatible API in Xcriminal"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Xcriminal registers it as the
...
```bash
Xcriminal onboard --auth-choice synthetic-api-key
```
...
Note: Xcriminal's Anthropic client appends `/v1` to the base URL, so use
