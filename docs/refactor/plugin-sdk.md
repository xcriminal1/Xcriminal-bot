...
Delivery:
- Publish as `@Xcriminal/plugin-sdk` (or export from core under `Xcriminal/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `XcriminalPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: XcriminalConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: XcriminalConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: XcriminalConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: XcriminalConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: XcriminalConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: XcriminalConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@Xcriminal/plugin-sdk`.
- Add `api.runtime` to `XcriminalPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `XcriminalRuntime: ">=2026.2.0"`).
