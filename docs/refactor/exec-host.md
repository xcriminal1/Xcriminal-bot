...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.Xcriminal/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.Xcriminal/node.json` (node id + pairing token).
...
Path: `~/.Xcriminal/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.Xcriminal/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.Xcriminal/exec-approvals.sock` (0600).
