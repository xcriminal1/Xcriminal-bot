---
summary: "First-run onboarding flow for Xcriminal (macOS app)"
read_when:
...
- Writes credentials to `~/.Xcriminal/credentials/oauth.json`
...
The app can install the global `Xcriminal` CLI via npm/pnpm so terminal
...
On the first agent run, Xcriminal bootstraps a workspace (default `~/clawd`):
...
```bash
Xcriminal webhooks gmail setup --account you@gmail.com
```
...
- `~/.Xcriminal/credentials/oauth.json`
