...
service via the external `Xcriminal` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`Xcriminal_PROFILE`; legacy `com.Xcriminal.*` is supported).
