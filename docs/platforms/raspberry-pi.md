---
summary: "Xcriminal on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Xcriminal on a Raspberry Pi
  - Running Xcriminal on ARM devices
  - Building a cheap always-on personal AI
---

# Xcriminal on Raspberry Pi

## Goal

Run a persistent, always-on Xcriminal Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Xcriminal features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled Xcriminal
...
```bash
# Check logs
journalctl -u Xcriminal --no-pager -n 100

# Common fix: rebuild
cd ~/Xcriminal  # if using hackable install
npm run build
sudo systemctl restart Xcriminal
```
