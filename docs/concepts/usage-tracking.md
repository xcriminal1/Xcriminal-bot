...
- `/usage cost` in chats: local cost summary aggregated from Xcriminal session logs.
- CLI: `Xcriminal status --usage` prints a full per-provider breakdown.
- CLI: `Xcriminal channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
