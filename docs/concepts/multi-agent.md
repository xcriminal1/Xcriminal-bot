...
- **Session store** (chat history + routing state) under `~/.Xcriminal/agents/<agentId>/sessions`.
...
available from `~/.Xcriminal/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.Xcriminal` (or `Xcriminal_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.Xcriminal/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.Xcriminal/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Xcriminal runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `Xcriminal_PROFILE` is set).
- State defaults to `~/.Xcriminal/agents/main/agent`.
...
```bash
Xcriminal agents list --bindings
```
...
`~/.Xcriminal/Xcriminal.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.Xcriminal/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.Xcriminal/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.Xcriminal/credentials/whatsapp/personal
          // authDir: "~/.Xcriminal/credentials/whatsapp/personal",
        },
        biz: {
