...
## Quick check: `Xcriminal security audit` (formerly `Xcriminal security audit`)
...
# (On older installs, the command is `Xcriminal ...`.)
```
...
- Tighten local perms (`~/.Xcriminal` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

Xcriminal is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Xcriminal also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.Xcriminal/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.Xcriminal/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.Xcriminal/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.Xcriminal/credentials/oauth.json`
...
`Xcriminal security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${Xcriminal_GATEWAY_PASSWORD}
```
...
Xcriminal stores session transcripts on disk under `~/.Xcriminal/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.Xcriminal` (see the audit section below). If you need
...
Xcriminal can refresh the skills list mid-session:
...
- If you install plugins from npm (`Xcriminal plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.Xcriminal/extensions/<pluginId>/` (or `$Xcriminal_STATE_DIR/extensions/<pluginId>/`).
  - Xcriminal uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
Xcriminal pairing approve <channel> <code>
```
...
By default, Xcriminal routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Xcriminal has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.Xcriminal/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.Xcriminal or your logs.”
...
4. **Re-run audit**
   - `Xcriminal security audit --deep` and confirm the report is clean.
...
- `~/.Xcriminal/Xcriminal.json`: `600` (user read/write only)
- `~/.Xcriminal`: `700` (user only)

`Xcriminal doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `Xcriminal_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_Xcriminal-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `Xcriminal_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `Xcriminal doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `Xcriminal_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `Xcriminal_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Xcriminal
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Xcriminal verifies the identity by resolving the
...
- Xcriminal will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.Xcriminal/` (or `$Xcriminal_STATE_DIR/`) may contain secrets or private data:

- `Xcriminal.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `Xcriminal status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.Xcriminal/sandboxes`
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `Xcriminal gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `Xcriminal_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/Xcriminal/Xcriminal-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.Xcriminal/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Xcriminal version
...
Found a vulnerability in Xcriminal? Please report responsibly:
