...
- Replaces the legacy `gateway` command. CLI entry point: `Xcriminal gateway`.
...
Xcriminal gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
Xcriminal gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.Xcriminal/Xcriminal.json` (or `Xcriminal_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__Xcriminal__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `Xcriminal_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `Xcriminal_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `Xcriminal_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.Xcriminal.*` may still exist)
- Linux: `Xcriminal-gateway-<profile>.service`
- Windows: `Xcriminal Gateway (<profile>)`

Install metadata is embedded in the service config:
- `Xcriminal_SERVICE_MARKER=Xcriminal`
- `Xcriminal_SERVICE_KIND=gateway`
- `Xcriminal_SERVICE_VERSION=<version>`
...
```bash
Xcriminal --dev setup
Xcriminal --dev gateway --allow-unconfigured
# then target the dev instance:
Xcriminal --dev status
Xcriminal --dev health
```

Defaults (can be overridden via env/flags/config):
- `Xcriminal_STATE_DIR=~/.Xcriminal-dev`
- `Xcriminal_CONFIG_PATH=~/.Xcriminal-dev/Xcriminal.json`
- `Xcriminal_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `Xcriminal_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `Xcriminal_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `Xcriminal_CONFIG_PATH`
- unique `Xcriminal_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
Xcriminal --profile main gateway install
Xcriminal --profile rescue gateway install
```

Example:
```bash
Xcriminal_CONFIG_PATH=~/.Xcriminal/a.json Xcriminal_STATE_DIR=~/.Xcriminal-a Xcriminal gateway --port 19001
Xcriminal_CONFIG_PATH=~/.Xcriminal/b.json Xcriminal_STATE_DIR=~/.Xcriminal-b Xcriminal gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `Xcriminal health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `Xcriminal`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `Xcriminal gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.Xcriminal.*` is cleaned up).
  - `Xcriminal doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Xcriminal profile services.
  ...
  - Cleanup: `Xcriminal gateway uninstall` (current service) and `Xcriminal doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `Xcriminal gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Xcriminal.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.Xcriminal.*` labels still unload cleanly).
- To stop it cleanly, use `Xcriminal gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `Xcriminal gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `Xcriminal gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
Xcriminal installs a **systemd user service** by default on Linux/WSL2. We
...
`Xcriminal gateway install` writes the user unit. `Xcriminal doctor` audits the
...
Restart=always
RestartSec=5
Environment=Xcriminal_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/Xcriminal-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now Xcriminal-gateway[-<profile>].service
```
...
## CLI helpers
- `Xcriminal gateway health|status` — request health/status over the Gateway WS.
- `Xcriminal message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `Xcriminal agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `Xcriminal gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `Xcriminal gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `Xcriminal gateway` and the legacy TCP control port.
