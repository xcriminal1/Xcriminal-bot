...
- During heartbeat runs, Xcriminal treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `Xcriminal sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Xcriminal skips the heartbeat run entirely (no model call).
...
```bash
Xcriminal system event --text "Check for urgent follow-ups" --mode now
```
