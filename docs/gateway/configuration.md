---
summary: "All configuration options for ~/.Xcriminal/Xcriminal.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Xcriminal reads an optional **JSON5** config from `~/.Xcriminal/Xcriminal.json` (comments + trailing commas allowed).

If the file is missing, Xcriminal uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Xcriminal only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `Xcriminal doctor`, `Xcriminal logs`, `Xcriminal health`, `Xcriminal status`, `Xcriminal service`, `Xcriminal help`).
- Run `Xcriminal doctor` to see the exact issues.
- Run `Xcriminal doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `Xcriminal config set`. Keep a backup of `~/.Xcriminal/Xcriminal.json`.
...
```bash
Xcriminal gateway call config.get --params '{}' # capture payload.hash
Xcriminal gateway call config.apply --params '{
...
```bash
Xcriminal gateway call config.get --params '{}' # capture payload.hash
Xcriminal gateway call config.patch --params '{
...
{ "$include": "/etc/Xcriminal/base.json5" } // absolute
...
```json5
// ~/.Xcriminal/clients/mueller/agents.json5
[
...
```json5
// ~/.Xcriminal/clients/mueller/broadcast.json5
{
...
Xcriminal reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Xcriminal runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `Xcriminal_LOAD_SHELL_ENV=1`
- `Xcriminal_SHELL_ENV_TIMEOUT_MS=15000`
...
Xcriminal stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.Xcriminal/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.Xcriminal/credentials/oauth.json` (or `$Xcriminal_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.Xcriminal/agent/*` (migrated by `Xcriminal doctor` into `~/.Xcriminal/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `Xcriminal_OAUTH_DIR`
- Agent dir (default agent root override): `Xcriminal_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Xcriminal imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Xcriminal derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/Xcriminal/Xcriminal-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/Xcriminal/Xcriminal.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/Xcriminal/Xcriminal.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.Xcriminal/credentials/whatsapp/biz
          // authDir: "~/.Xcriminal/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `Xcriminal doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "Xcriminal", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.Xcriminal/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `Xcriminal.json`).
...
Xcriminal starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Xcriminal starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `Xcriminal plugins install @Xcriminal/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Xcriminal starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Xcriminal spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Xcriminal
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/Xcriminal" } }
}
```
...
When a file exceeds this limit, Xcriminal logs a warning and injects a truncated
...
message envelopes). If unset, Xcriminal uses the host timezone at runtime.
...
`[Xcriminal]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[Xcriminal]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[Xcriminal]"`, Xcriminal will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Xcriminal generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.Xcriminal/settings/tts.json",
      elevenlabs: {
...
Xcriminal also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Xcriminal currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `Xcriminal configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:Xcriminal`: all built-in Xcriminal tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.Xcriminal/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.Xcriminal/sandboxes",
        docker: {
          ...
          apparmorProfile: "Xcriminal-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "Xcriminal-sandbox-browser:bookworm-slim",
          containerPrefix: "Xcriminal-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Xcriminal uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.Xcriminal/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Xcriminal config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Xcriminal writes/merges a `models.json` into
`~/.Xcriminal/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Xcriminal uses
...
- Shortcut: `Xcriminal onboard --auth-choice opencode-zen`.
...
Shortcut: `Xcriminal onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `Xcriminal onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `Xcriminal onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `Xcriminal onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `Xcriminal onboard --auth-choice minimax-api`.
...
- Override the agent config root with `Xcriminal_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.Xcriminal/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.Xcriminal/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.Xcriminal/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Xcriminal stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.Xcriminal/skills` (workspace skills
...
from `~/.Xcriminal/extensions`, `<workspace>/.Xcriminal/extensions`, plus any
...
Xcriminal can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Xcriminal.app menubar, or `Xcriminal gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/Xcriminal" }
...
- Examples: `"/ui"`, `"/Xcriminal"`, `"/apps/Xcriminal"`.
...
- When a connection comes from one of these IPs, Xcriminal uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `Xcriminal gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `Xcriminal_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `Xcriminal_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Xcriminal
...
macOS app behavior:
- Xcriminal.app watches `~/.Xcriminal/Xcriminal.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.Xcriminal/Xcriminal.json` (or `Xcriminal_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.Xcriminal/Xcriminal.json` (or `Xcriminal_CONFIG_PATH`)
...
- `Xcriminal_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `Xcriminal --dev …` → uses `~/.Xcriminal-dev` + shifts ports from base `19001`
- `Xcriminal --profile <name> …` → uses `~/.Xcriminal-<name>` (port via config/env/flags)
...
Gmail helper config (used by `Xcriminal webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "Xcriminal@gmail.com",
...
- Set `Xcriminal_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Xcriminal defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__Xcriminal/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__Xcriminal__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `Xcriminal_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.Xcriminal/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.Xcriminal/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.Xcriminal/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_Xcriminal-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_Xcriminal-bridge._tcp` under `~/.Xcriminal/dns/` using the standard discovery domain `Xcriminal.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `Xcriminal.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `Xcriminal.internal` via that server

One-time setup helper (gateway host):

```bash
Xcriminal dns setup --apply
```
