import type { XcriminalApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as XcriminalApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as XcriminalApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as XcriminalApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as XcriminalApp);
    await loadDevices(host as unknown as XcriminalApp);
    await loadConfig(host as unknown as XcriminalApp);
    await loadExecApprovals(host as unknown as XcriminalApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as XcriminalApp);
    await loadConfig(host as unknown as XcriminalApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as XcriminalApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as XcriminalApp, { reset: true });
    scheduleLogsScroll(
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as XcriminalApp, false),
    loadPresence(host as unknown as XcriminalApp),
    loadSessions(host as unknown as XcriminalApp),
    loadCronStatus(host as unknown as XcriminalApp),
    loadDebug(host as unknown as XcriminalApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as XcriminalApp, true),
    loadConfigSchema(host as unknown as XcriminalApp),
    loadConfig(host as unknown as XcriminalApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as XcriminalApp, false),
    loadCronStatus(host as unknown as XcriminalApp),
    loadCronJobs(host as unknown as XcriminalApp),
  ]);
}
